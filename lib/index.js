(function (global, factory) {
    ...
    function Socket(config) {
        ...
        this._onClose = function (e) {
            // 1000:	// CLOSE_NORMAL
            _this._log('socket closed. error code: ', e.code);
        ...
        this.close = function () {
            ...
            if (_this._reconnectTimeout) {
                clearInterval(_this._reconnectTimeout);
            }
            if (_this._isConnected()) {
                _this._log('closing socket');
                _this._socket && _this._socket.close();
            } else {
        ...
        this._attemptReconnect = function () {
            ...
            _this._log('attempting reconnect in: ' + time + 'ms');

            _this._reconnectTimeout = setTimeout(function () {

                _this._log('reconnecting');

                _this._eventer.emit(_this.RECONNECTING);

                !_this._isConnected() && _this.open();
            }, time);
        };
